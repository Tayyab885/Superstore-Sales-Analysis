...
This project is focused on analyzing the sales data of a superstore. The analysis is performed in two parts: pre-processing the dataset using Python and Pandas, and analyzing the data using SQL.

## Dataset

The dataset used for this analysis is a superstore sales dataset. It contains information about the sales made by the store, including details such as order date, ship date, customer details, product details, and sales revenue. The dataset contains 9994 rows and 13 columns.

## Pre-processing the Dataset

Before analyzing the data, the dataset needs to be cleaned and pre-processed to ensure the accuracy and reliability of the analysis. The following steps were performed using Python and Pandas:

1. Removing duplicate rows: Duplicate rows in the dataset, excluding the Row_ID column, were identified and removed. This step helps in eliminating redundant data and avoiding bias in the analysis.

2. Removing rows with missing values: Rows that have missing values were removed from the dataset. Missing values can affect the accuracy of the analysis and lead to incorrect conclusions.

3. Removing irrelevant values: Any irrelevant values in each column were identified and removed. This step ensures that the dataset contains only relevant and meaningful data for analysis.

4. Validating data values: Each column was validated to ensure that the values are in the correct format. In particular, the order date and ship date values were checked to ensure they are in the proper date format. Additionally, it was verified that the ship date is greater than or equal to the order date, as this is a logical requirement.

After pre-processing, the cleaned dataset was exported as a CSV file, which serves as the input for the subsequent analysis.
...
8. Which order has the highest cumulative value?
   - This query identifies the order with the highest cumulative value, providing an understanding of the most
